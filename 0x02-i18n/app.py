...
def before_request() -> None:
    ...
    tym = now_time.astimezone(timezone(get_timezone()))
    locale.setlocale(locale.LC_TIME, (get_locale(), 'UTF-8'))
    time_format = "%b %d, %Y %I:%M:%S %p"
    g.time = tym.strftime(time_format)
...
def get_timezone() -> str:
    ...
    tymzone = request.args.get('timezone', None)
    if tymzone:
        try:
            return timezone(tymzone).zone
        except p_ex.UnknownTimeZoneError:
            pass

    # Secondly, find the timezone from user's settings
    if g.user:
        tymzone = g.user.get('timezone')
        if tymzone:
            try:
                return timezone(tymzone).zone
            except p_ex.UnknownTimeZoneError:
...
def index() -> str:
    ...
    """
    return render_template('index.html', current_time=g.time)
