...
describe('createPushNotificationsJobs', () => {
  const PARENT = sinon.spy(console);
  const QUEUE = createQueue({ name: 'test_push_notification_code' });
  ...
  afterEach(() => {
    PARENT.log.resetHistory();
  });

  it('shows error message if jobs is not an array', () => {
    expect(
      createPushNotificationsJobs.bind(createPushNotificationsJobs, {}, QUEUE)
    ).to.throw('Jobs is not an array');
  });

  it('Enqueue jobs with the correct type', (done) => {
    expect(QUEUE.testMode.jobs.length).to.equal(0);
    const jobInfos = [
      {
        phoneNumber: '44556677889',
        message: 'Use the code 1243 to verify your account',
      },
      {
        phoneNumber: '98877665544',
        message: 'Use the code 1192 to verify your account',
      },
    ];
    ...
    QUEUE.process('push_notification_code_3', () => {
      expect(
        PARENT.log
          .calledOnceWith('Notification job created:', QUEUE.testMode.jobs[0].id)
      ).to.be.false;
      done();
    });
  });
  ...
  it('registers the progress event handler for a job', (done) => {
    QUEUE.testMode.jobs[0].addListener('progress', () => {
      expect(
        PARENT.log
          .calledOnceWith('Notification job', QUEUE.testMode.jobs[0].id, '25% complete')
      ).to.be.false;
      done();
    });
  ...
  it('registers the failed event handler for a job', (done) => {
    QUEUE.testMode.jobs[0].addListener('failed', () => {
      expect(
        PARENT.log
          .calledOnceWith('Notification job', QUEUE.testMode.jobs[0].id, 'failed:', 'Failed to send')
      ).to.be.false;
      done();
    });
  ...
  it('registers the complete event handler for a job', (done) => {
    QUEUE.testMode.jobs[0].addListener('complete', () => {
      expect(
        PARENT.log
          .calledOnceWith('Notification job', QUEUE.testMode.jobs[0].id, 'completed')
      ).to.be.false;
      done();
    });
